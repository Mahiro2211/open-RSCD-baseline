...
if __name__ == '__main__':
    ...
    from configs._base_ import data_module
    ...
    p = r'D:\coding\open-remote-sensing-detection-baseline\_dataset_\data\LEVIR\LEVIR-Cropped'

    data_set = CDDataset(p, img_size=256)
    train_loader = DataLoader(data_set, batch_size=4)
    # loss = cross_entropy()
    net = Net(in_ch=3, out_ch=2).cuda()
    for batch in tqdm(train_loader):
        imgA, imgB, label = batch['A'].cuda(), batch['B'].cuda(), batch['L'].cuda()
        preds = net(imgA, imgB)
        loss1 = cross_entropy(preds, label)